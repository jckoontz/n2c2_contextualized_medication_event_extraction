from transformers import BertModel, LongformerModel, AdamW, get_linear_schedule_with_warmup
...
class NER_CRF(pl.LightningModule):
  ...
  def training_step(self, batch, batch_idx):
      ...
      loss = -self.crf(logits, labels, mask=attention_masks.bool()) / float(len(batch))
      best_path = self.crf.decode(logits, mask=attention_masks.bool())
      pred, true = [], []
      for path, mask, label in zip(best_path, attention_masks, labels): 
        idx = len(torch.where(mask.bool() == True)[0].detach().cpu().numpy().tolist())
      ...
      self.log("performance", {"train_loss": loss, "train_acc": accuracy, 
